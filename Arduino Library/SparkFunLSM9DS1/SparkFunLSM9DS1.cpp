...
{
	sWire.begin();	// Initialize I2C library
}

// Wire.h read and write protocols
void LSM9DS1::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
	sWire.beginTransmission(address);  // Initialize the Tx buffer
	sWire.write(subAddress);           // Put slave register address in Tx buffer
	sWire.write(data);                 // Put data in Tx buffer
	sWire.endTransmission();           // Send the Tx buffer
}
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	...
	sWire.beginTransmission(address);         // Initialize the Tx buffer
	sWire.write(subAddress);	                 // Put slave register address in Tx buffer
	sWire.endTransmission(true);             // Send the Tx buffer, but send a restart to keep connection alive
	sWire.requestFrom(address, (uint8_t) 1);  // Read one byte from slave register address
	while ((sWire.available() < 1) && (timeout-- > 0))
		delay(1);

	if (timeout <= 0)
		return 255;	//! Bad! 255 will be misinterpreted as a good value.

	data = sWire.read();                      // Fill Rx buffer with result
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	sWire.write(subAddress | 0x80);     // Put slave register address in Tx buffer

	sWire.endTransmission(true);             // Send the Tx buffer, but send a restart to keep connection alive
	uint8_t i = 0;
	sWire.requestFrom(address, count);  // Read bytes from slave register address
	while ((sWire.available() < count) && (timeout-- > 0))
		delay(1);
	if (timeout <= 0)
		return -1;
